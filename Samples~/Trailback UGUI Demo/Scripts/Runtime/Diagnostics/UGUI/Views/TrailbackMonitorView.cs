...
namespace ModularForge.Trailback.Demo
{
    ...
    public class TrailbackMonitorView : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            var state = TrailbackFacade.GetState();
            Refresh(state);
        ...
        private void UpdateTrailbackState(TrailbackState state)
        {
            ...
            _stateBuilder.AppendLine($"Current: {state.NavigationSnapshot.Current.DisplayName}");
            _stateBuilder.AppendLine($"Previous: {state.NavigationSnapshot.BackTarget.DisplayName}");

            _stateBuilder.AppendLine($"Can Go Back: {state.NavigationSnapshot.CanGoBack}");

            if (state.NavigationSnapshot.BlockReason != BackBlockReason.None)
            {
                _stateBuilder.AppendLine();

                _stateBuilder.AppendLine($"Reason: {state.NavigationSnapshot.BlockReason}");

                if (!string.IsNullOrWhiteSpace(state.NavigationSnapshot.BlockDetails))
                {
                    _stateBuilder.AppendLine($"Details: {state.NavigationSnapshot.BlockDetails}");
                }
            }
